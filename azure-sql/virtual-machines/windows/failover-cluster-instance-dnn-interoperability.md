---
title: Feature Interoperability with SQL Server FCI and DNN
description: "Learn about the considerations when working with certain SQL Server features and a distributed network name (DNN) resource with a failover cluster instance on SQL Server on Azure VMs. "
author: AbdullahMSFT
ms.author: amamun
...
# Feature interoperability with SQL Server FCI and DNN

[!INCLUDE [appliesto-sqlvm](../../includes/appliesto-sqlvm.md)]

[!INCLUDE [tip-for-multi-subnet-ag](../../includes/virtual-machines-ag-listener-multi-subnet.md)]

There are certain SQL Server features that rely on a hard-coded virtual network name (VNN). As such, when using the distributed network name (DNN) resource with your failover cluster instance and SQL Server on Azure VMs, there are some considerations.

In this article, learn how to configure the network alias when using the DNN resource, and which SQL Server features require further consideration.
...
Follow the steps in [Create or delete a server alias for use by a client](/sql/database-engine/configure-windows/create-or-delete-a-server-alias-for-use-by-a-client) to create an alias that maps the VNN to the DNN DNS name.
...
Users of [SQL Server Management Studio](/sql/ssms/sql-server-management-studio-ssms), [sqlcmd](/sql/tools/sqlcmd-utility), the [MSSQL extension for Visual Studio Code](/sql/tools/visual-studio-code-extensions/mssql/mssql-extension-visual-studio-code), and [SQL Server Data Tools](/sql/ssdt/sql-server-data-tools) need to explicitly specify the DNN DNS name as the server name in the connection string.
...
:::image type="content" source="media/failover-cluster-instance-dnn-interoperability/alias-in-configuration-manager.png" alt-text="Screenshot of configuring the DNN DNS name as the network alias using SQL Server Configuration Manager.":::

Use the full instance name for a named instance, like the following image example:

:::image type="content" source="media/failover-cluster-instance-dnn-interoperability/alias-named-instance-configuration-manager.png" alt-text="Screenshot showing the full instance name when configuring a network alias for a named instance.":::
...
> [!TIP]  
> The DNN defined in the FCI doesn't replace the Azure Load Balancer requirement for the clustered MSDTC.

## FILESTREAM

Though FILESTREAM is supported for a database in an FCI, accessing FILESTREAM or FileTable by using File System APIs with DNN isn't supported.
...
Alternatively, you can create the linked server using the virtual network name (VNN) instead, but you then need to define a network alias to map the VNN to the DNN.
...
SQL Server 2019 CU2 and later versions.

### What is the expected failover time when DNN is used?

For DNN, the failover time is just the FCI failover time, without any time added (like probe time when you're using Azure Load Balancer).
