...
| <a id="pvs_cleanup_lock"></a> `PVS_CLEANUP_LOCK` | Occurs when the persistent version store (PVS) cleanup process is waiting for a lock required to start the cleanup. Might occur when an active transaction is preventing PVS cleanup. For more information, see [Monitor and troubleshoot accelerated database recovery](../accelerated-database-recovery-troubleshoot.md).<br /><br />**Applies to**: [!INCLUDE [ssSQL15-md](../../includes/sssql15-md.md)] and later versions. |
