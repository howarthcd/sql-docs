...
| **SQL database in [!INCLUDE [fabric](../includes/fabric.md)]** | Fully managed <sup>4</sup> | [SQL database in Fabric](/fabric/database/sql/overview) is a developer-friendly, AI-ready, operational database based on the same SQL Database Engine as found in Azure SQL Database. For more information, see [Features comparison: Azure SQL Database and SQL database in Fabric](/fabric/database/sql/limitations#features-of-azure-sql-database-and-fabric-sql-database). |
