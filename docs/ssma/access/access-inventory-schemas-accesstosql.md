...
| `ValidationRule` | **nvarchar(4000)** | The rule that defines valid input for the table. If no validation rule exists, the field contains an empty string. |
...
| `IsNullable` | **bit** | Specifies if the column can contain null values. If the value is 1, the column can contain null values. If the value is 0, the column can't contain null values. The validation rule can also be used to prevent null values. |
...
- [Export an Access inventory](exporting-an-access-inventory-accesstosql.md)
