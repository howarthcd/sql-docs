...
The default actions specify default settings for refreshing objects from the Access database and for synchronizing objects with the SQL Server database. For more information, see [Refresh from Database](refresh-from-database-accesstosql.md)

You can access two different Synchronization pages that contain the same settings:

- To specify settings for all future SSMA projects, on the **Tools** menu, select **DefaultProject Settings**, and then select **Loading Objects** at the bottom of the left pane.

- To specify settings for the current project, on the **Tools** menu, select **Project Settings**, and then select **Loading Objects** at the bottom of the left pane.
