...
## Remarks
 ...
 - `IDENT_CURRENT` is not limited by scope and session; it is limited to a specified table. `IDENT_CURRENT` returns the identity value generated for a specific table in any session and any scope. For more information, see [IDENT_CURRENT (Transact-SQL)](ident-current-transact-sql.md).  
