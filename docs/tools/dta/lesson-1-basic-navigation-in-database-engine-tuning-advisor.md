...
Instructions for restoring databases in SSMS are here: [Restore a Database Backup Using SSMS](../../relational-databases/backup-restore/restore-a-database-backup-using-ssms.md)
...
1. Once SSMS is open, select the **Tools** menu and select **Database Tuning Advisor**.
...
1. In the **Options** dialog box, view the following options:
   ...
   - Expand the **On startup** list to view what Database Engine Tuning Advisor can display when it's started. By default, **Show a new session** is selected.
   ...
   - The **Number of items in most recently used lists** can be set between `1` and `10`. This sets the maximum number of items in the lists displayed by selecting **Recent Sessions** or **Recent Files** on the **File** menu. By default, this option is set to `4`.
